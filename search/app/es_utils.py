...
#         LEFT JOIN film_genres fg ON f.id = fg.film_id
#         LEFT JOIN genre g ON g.id = ANY(fg.genre_ids)
#         GROUP BY f.id, f.title,f.vote_count, f.average_rating
#     """)
#     movies = cur.fetchall()
#     cur.close()
#     conn.close()
...
#     """Подготавливает данные для индексации в Elasticsearch."""
#     documents = []
#     for movie_id, title, vote_count, average_rating, genres in movies:
#         doc = {
#             "movieID": movie_id,
#             "title": title,
#             "vote_count": vote_count,
#             "average_rating": average_rating,
#             "genres": genres if genres else [], # Ensure an empty list is set when the movie has no genres
#         }
#         documents.append(doc)
#     return documents
